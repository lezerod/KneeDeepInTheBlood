...
public class GameUpdateThread extends Thread implements EventList {
			...
			if (gameWorld.getThreadTicks() > (gameWorld.getMinutesToWin() * 60 * 1000 / GameSettings.THREADTICKTIME)) {
				...
				Platform.runLater(new Runnable() {
					...
					public void run() {
						...
						alert.setHeaderText("Herzlichen Glueckwunsch! Sie haben das Spiel gewonnen.");
						alert.setContentText("Moechten sie noch einmal spielen?");
						gameIsRunning = false;
						alert.showAndWait();
					...
					if (gameWorld.getLeben() <= 0) {
						...
						Platform.runLater(new Runnable() {
							...
							public void run() {
								...
								alert.setContentText("Moechten sie nocheinmal spielen?");
								gameIsRunning = false;
								alert.showAndWait();
								if (alert.getResult() == ButtonType.YES) {
									view.switchScene(view.getSceneNewGame());
	...
	public void raiseKeyDownEvent(KeyEvent e) {
		...
		if (e.getCode() == KeyCode.ESCAPE) {
			...
			alert.setHeaderText("Moechten sie das Spiel wirklich beenden?");
